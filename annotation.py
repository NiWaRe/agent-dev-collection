...
@st.cache_resource
def assemble_feedback_data(entity:str, project_name: str, op_name: str) -> pd.DataFrame:
    data = []
    client = start_weave(entity, project_name)
    thumbs_down = client.feedback(reaction=feedback_name)
    calls = thumbs_down.refs().calls()
    for call in calls:
        # only take specifc ops so that parsing logic works in next step
        if op_name not in call.op_name:
            break

        # get last feedback for now
        ...
        # NOTE: this can be used to parse logged production calls from chatbot.py
        # prediction - used as question and target answer and url in dataset
        data.append({
            "query": call.inputs['prompt'],
            "prediction": call.output[0],
            "true_main_source": "https://www.ipcc.ch/report/ar6/syr/downloads/report/IPCC_AR6_SYR_LongerReport.pdf",
...
if __name__ == "__main__":
    ...
    project_name = st.text_input("Enter Weave Project Name", "weave-cookboook-demo")
    ...
    op_name = st.text_input("Enter Weave URL", "weave:///wandb-smle/weave-cookboook-demo/op/production_chat_response:TwdLxqtOgUqmeL0Ebxe9bPuH5rdJe9ZoNWC5rY3UrxM")
    ...
    if edit_mode:
        if data_selector == "Weave Dataset (existing)":
            ...
            weave_dataset_df = assemble_feedback_data(entity, project_name, op_name)
        else:
...
# NOTE: this can be used to parse the rated evaluation runs
        # data.append({
        #     # prediction - used as question and target answer and url in dataset
        #     "query": call.inputs['example']['query'],
        #     "prediction": call.output["model_output"]["result"]["content"],
        #     "used_main_source": call.output["model_output"]["source_documents"][0]["page_content"],
